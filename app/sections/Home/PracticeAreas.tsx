      ...
      <Container>
        ...
        <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-6 xl:gap-7" stagger={0.08} amount={0.08}>
          {PRACTICE_AREAS.map((area) => {
            ...
            return (
              <motion.div key={area.id} variants={staggerItemVariants}>
                <Link
                  href={`/practice-areas/${area.slug}`}
                  className="bg-white rounded-xl p-6 md:p-7 xl:p-10 transition-all duration-300 hover:bg-secondary hover:-translate-y-1 hover:shadow-xl group no-underline focus:outline-none focus:ring-2 focus:ring-secondary focus:ring-offset-2 focus:ring-offset-primary block"
